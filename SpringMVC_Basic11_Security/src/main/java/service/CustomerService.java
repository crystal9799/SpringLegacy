...
public class CustomerService {
	...
	public String noticeReg(Notice n, HttpServletRequest request, Principal principal) {
		...
		/*
		 SecurityContext context = SecurityContextHolder.getContext(); //Spring
		 Security의 모든 정보를 가져옴 //인증에관련된 것만 추출 Authentication auth =
		 context.getAuthentication(); UserDetails userinfo =
		 (UserDetails)auth.getPrincipal(); //parameter에 principal을 넣으면 , 위 세줄의 코드를
		 생략가능
		  
		 //인증된 사용자에 대한 권한정보 (return Collection) System.out.println("권한정보 : " +
		 userinfo.getAuthorities()); //인증된 사용자 아이디 System.out.println("사용자ID : " +
